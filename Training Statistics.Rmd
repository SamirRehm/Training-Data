...
p <- plot_ly(width = 850, height = 500, type = "bar", hoverinfo = "all") %>%
  ...
  add_trace(x = RunningData[RunningData$RunType == "Work",]$Date, y = RunningData[RunningData$RunType == "Work",]$Distance, text = RunningData[RunningData$RunType == "Work",]$Pace, textposition = 'auto',  marker = list(line = list(color = 'rgb(8,48,107)', width = 2), color = 'rgba(100,100, 255,0.7)'), name = "Run to or from work", hoverinfo = "none") %>%
  
    add_trace(x = RunningData[RunningData$RunType=="Warm-up",]$Date,y=RunningData[RunningData$RunType == "Warm-up",]$Distance, text = RunningData[RunningData$RunType == "Warm-up",]$Pace, textposition = 'auto', marker = list(color = 'rgba(100,255,100,0.7)', line = list(color = 'rgb(8,48,107)', width = 2)), name = "Warm-up Run") %>%
  
  add_trace(x = RunningData[RunningData$RunType == "Steady",]$Date, y=RunningData[RunningData$RunType == "Steady",]$Distance, text = RunningData[RunningData$RunType == "Steady",]$Pace, textposition = 'auto', marker = list(color = 'rgba(255, 100, 100, 0.7)', line = list(color = 'rgb(8,48,107)', width = 2)), name = "Steady-State Run") %>%
  
  add_trace(x = RunningData[RunningData$RunType == "Rest",]$Date, y = RunningData[RunningData$RunType == "Rest",]$Distance, text = RunningData[RunningData$RunType == "Rest",]$Pace, textposition = 'auto', marker = list(color = 'rgba(255, 255, 100, 0.7)', line = list(color = 'rgb(8,48,107)', width = 2)), name = "Recovery Run") %>%
  
  layout(barmode = "stack",  title = paste("Training Plan Week 1 -", signif(sum(RunningData$Distance)/1000, 4), "km"), xaxis = list(title = "Date"))
p
```
