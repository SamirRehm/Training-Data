...
p <- plot_ly(width = 850, height = 515, type = "bar", hoverinfo = "all") %>%
  ...
  layout(barmode = "stack",  title = paste("<b>Training Plan Week 1 -", signif(sum(RunningData$Distance), 4), "km", "</b>"), xaxis = list(title = "Date"), yaxis = list(title = "Distance (km)"))
p
```
\newline 
&nbsp;  
...
google_map(key = map_key, data = RunningData, width = 1000) %>%
  add_polylines(polyline = "Polylines", stroke_colour = "Day", stroke_weight = 4, stroke_opacity = 0.8, legend = T, palette = lstPalette)

```
Note - Track Intervals are excluded as they were tracked manually